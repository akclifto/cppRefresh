...
Signals are interrupts delivered to a process by the OS which can terminate a program prematurely. These are generated by pressing CTRL+C on Windows and Linux.

Some signals cannot be caught, but some can and here is a list to take appropriate actions. These are defined in the C++ library header file `<csignal>`:

- `SIGABRT` : Abnormal termination of the program, such as a call to abort.
- `SIGFPE` : An error in arithmetic operation, such as a divide by zero or overflow operation.
- `SIGILL` : Detenction of illegal instruction.
...
The function received two args, the first (int sig) represents signal number and second arg as a pointer to the signal-handling function.

Simple example to catch SIGINT using signal(). Whatever signal you want to catch, you must register it first using signal() and associate it with signal handler:
...
While this is running, press CTRL+C to interrupt the program. Will output `Interrupt signal (2) received.`. If you've used Linux, git or any variant, This is likely already been used to interrupt some operation that froze or hung when coding.

#### The raise() function

You can generate signals by using the raise() function; this takes an int signal number as an arg and used the following syntax:

`int raise (signal sig);`

`signal sig` is the signal number to send any of the signals: SIGINT, SIGABRT, SIGFPE, SIGILL, SIGSEGV, SIGTERM, SIGHUP. Example,
...
This will automatically interrupt the program once i == 3.
